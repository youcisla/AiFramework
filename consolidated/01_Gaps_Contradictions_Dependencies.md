...
| G8 | No incident path exists for development-time leaks, despite WS2 requiring reporting to Cybersecurity | A leak found in a prompt or a repository has no procedure, so it is likely to be handled informally | Security with Digital/IT | Reuse INSEAD's existing Open Talk / No Retaliation platform as the reporting route, with a named security contact and a service level |
...
| G10 | IP ownership for student-built work is unresolved | It determines how firmly the ownership transfer in Document 00 can be stated | Legal | Start from the INSEAD Copyright Policy and obtain a position on AI-generated student work, distinguishing coursework from institutional work |
